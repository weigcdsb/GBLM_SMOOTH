function [x, fv, nfun] = minfun(fun,x,opt,Ane,Bne,Aeq,Beq,lb,ub,dbg)
% gradient minimizer using sequential simplex
% function [x, fv, nfun] = minfun(fun,x,opt,Ane,Bne,Aeq,Beq,lb,ub,dbg)
%
...
%
% examples
% [x, fv, nfun] = minfun(@(x)(x(1)^2+x(2)^2),[-3 4],[],[1 0; 0 1],[-1; -1],[],[],[],[],1)
% [x, fv, nfun] = minfun(@(x)(x(1)^2+x(2)^2),[-3 4],[],[1 0],[-1],[0 1],[.2],[],[],1)
% [x, fv, nfun] = minfun(@(x)(x(1)^2+x(2)^2),[-3 4],[],...
%	{@(x)(x(1)); @(x)(x(2))},[-1; -1],[],[],[],[],1)
% [x, fv, nfun] = minfun(@(x)(x(1)^2+x(2)^2),[-3 4],[],[],[], ...
%	{@(x)(x(1)); @(x)(x(2))},[-1; -1],[],[],1)
...
bo  = 0.01;
% check for bound crossing (lower bound above upper bound) and ic in bounds
if exist('lb','var') && ~isempty(lb)
...
alpha = 1; beta = alpha * 2; gamma = alpha / 2; eta = 0.5; gs = 0.5;
...
% this while loop is not in the standard simplex documentation
% it is added for:
%	1) the inclusion of bounds / equality / inequality --> the gradients
%		are not always respected which can result in the simplex not finding 
%		a next step because one or more paramters are lock against a bound
%		thus a restart of the simplex is needed to drive a new search direction
%	2) the alpha / beta / ... terms are set to normal values but the gradient
%		search can be affected by these terms significcantly - particularly if
%		the function has local minimi scattered through out the design space
%	3) if unconstrained and a generally smooth function, this loop will only
%		execute once

% simplex scalar term adjustment factor (multiplier)
sc = 10;

% solution vector is the alpha, beta, eta, gamma, gs terms
% optimization is on the simplex scalar terms (alpha, beta, ...)
s = [alpha beta eta gamma gs]'; sp = Inf * s; fp = Inf;

% while the change in scalars is larger than threshold and
% the change in the optimal is larger than the threhold and
% the max number of functin calls is less than the limit
while max(abs(sp - s)) > opt.TolX && (fp - fv) > opt.TolFun ...
		&& nfun < opt.MaxIter
	
	% record last vector and function value (or initial guess if 1st loop)
	sp = s; fp = fv;
	
	% size gradient matrix and function vector
	xg = s * ones(1,6); fg = NaN(1,6);
	% build simplex terms gradients - fixed to a (sc) change
	st = s * sc; temp = 6:6:6*5;
% 	xg(temp) = xg(temp) + st';
	xg(temp) = st;
	
	% choose the optimization - base on ic or continual improvement
	if 0
		% re-start the solution for each simplex scalar case using the 
		% original initial condition (from prior loop of scalr cases)
		% test to see if initial gradient drives solution into a local min
		% more costly method as the simplex is solved 6 times.
		xx = x * ones(1,6); nn = zeros(1,6);
		for ii = 1:6
			if dbg; fprintf('case %g:	',ii); fprintf('%g	',xg(:,ii)); fprintf('\n'); end
			[fg(ii), xx(:,ii), nn(ii)] = simplex_loop(fv,xx(:,ii),nfun,fun,opt, ...
				len,bo,bf,lb,ub,Ane,Bne,Aeq,Beq, ...
				xg(1,ii),xg(2,ii),xg(3,ii),xg(4,ii),xg(5,ii),dbg);
	% 		disp(nn(ii))
		end
	else
		% re-start the solution for each simplex scalar case using the 
		% the simplex solution from the last scalar set (reduce function evals)
		% so obviously, the inital guess scalar case does most of the work
		% the other scalar cases optimized from the inital guess case solution
		% fewer function test but may get sucked into a local min early on and not
		% be able to recover
		xx = x * ones(1,6); nn = zeros(1,6);
		for ii = 1:6
			if dbg; fprintf('case %g:	',ii); fprintf('%g	',xg(:,ii)); fprintf('\n'); end
			...
			[fg(ii), xx(:,ii), nn(ii)] = simplex_loop(fv,xx(:,opc),nfun,fun,opt, ...
				len,bo,bf,lb,ub,Ane,Bne,Aeq,Beq, ...
				xg(1,ii),xg(2,ii),xg(3,ii),xg(4,ii),xg(5,ii),dbg);
%			disp(nn(ii))
		end
	end
% 	disp(fg); disp(xx); keyboard
	% gradient of function evals - which set of scalars is best?
	grd = (fg(2:end) - fg(1));
	% scaled by step size
	grd = grd ./ st';
	% normalized
	...
	% new simplex scalars from gradient
	s = s + sc * grd';
	
	% update solution value - minimum solution found and location
	[~, ii] = min(fg);
	fv = fg(ii); x = xx(:,ii);
% 	nfun = nn(ii);
	nfun = nfun + sum(nn);
