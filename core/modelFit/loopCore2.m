function [fit, fit_trace] = loopCore2(data, fit)
...
offset = ones(data.vecN, 1)*log(data.dt);
alph = glmfit([fit.Xc, fit.hist],data.post_spk_vec,'poisson','Offset',offset);
fit.wt_long = ones(data.vecN, 1)*alph(2);
fit.beta0 = ones(data.vecN,1)*alph(1);
fit.hist_beta = alph(3);
