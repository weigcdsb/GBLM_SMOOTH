function [fit,fit_trace] =...
    fit_model(data, fit, varargin)
...
for k = 1:iter
    ...
    if isfield(fit,'doFiltOnly') & fit.doFiltOnly
        ...
        [b, W, lam_pred, Qnew] = ppasmoo_poissexp(data.post_spk_vec, X, b0, W0, fit.F, fit.Q, offset);
        lam_pred = lam_pred'*data.dt;
        fit.llhd_pred = sum(-lam_pred + log((lam_pred+(lam_pred==0))).*(data.post_spk_vec'));
    end
    ...
    % use oracle short...
    if isfield(fit,'doOracleSTP') & fit.doOracleSTP
    ...
    toc
    if k>5 && abs(fit.dev-dev_prev)<toleranceValue;break;end
    dev_prev=fit.dev;
end
...
if isfield(fit,'doOracleSTP') & fit.doOracleSTP
    fit.covB = zeros(fit.stp_Nq+1);
else
    offset = fit.beta0 + log(data.dt) + fit.hist*fit.hist_beta;
    W_short = repmat(fit.wt_long.*fit.Xc, 1, fit.stp_Nq).*fit.stp_X;
    
    [alph,~,stat] = glmfit(W_short,data.post_spk_vec,'poisson','Offset',offset,'constant','off');
    fit.covB=stat.covb;
    
%     fit.wt_short_param = alph;
%     fit.wt_short = 1 + fit.stp_X*fit.wt_short_param;
